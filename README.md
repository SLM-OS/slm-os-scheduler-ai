...
### Fine-tune from SLM-OS captured traces (#879)

When SLM-OS has captured a scheduling-trace `.bin` via the on-target
`sched aitrace dump <path>` verb (SLM-OS issue #880), the
fine-tune flow adapts the synthetic-trained checkpoint to real
SLM-OS scheduling behaviour. Use a small learning rate and few epochs
— SLM-OS traces are short (hundreds-to-low-thousands of decisions),
so training from scratch on them would overfit. The pretrained
...
The trace format is fixed at SLM-OS-side v1 (`SCHED_TRACE_AI_VERSION =
1`); pin constants live in `data/slmos_traces.py`. If SLM-OS bumps the
record layout the version field must change in lockstep — the
`test_format_constants_pin` test asserts this.
