...
def write_config_h(n_actions: int, platform_name: str, output_dir: Path) -> None:
    """Write ai_config.h with platform-specific constants."""
    lines = [
...
def export_mlp(platform_name: str, n_actions: int, output_dir: Path) -> None:
    ...
    model_path = Path("models/mlp/best.pt")
    ...
    if model_n_actions != n_actions:
        print(f"  NOTE: model was trained with {model_n_actions} actions "
              f"(platform wants {n_actions})")
    ...
    print(f"  Loaded MLP: {model.param_count()} params, {n_actions} actions")
    ...
    # Write config with actual model n_actions
    write_config_h(n_actions, platform_name, output_dir)
...
def main():
    ...
    # Track the actual n_actions used (may differ from platform if model
    # was trained on mixed-platform data)
    actual_n_actions = n_actions

    for model_name in models:
        print(f"--- Exporting {model_name.upper()} ---")
        if model_name == "mlp":
            actual_n_actions = export_mlp(args.platform, n_actions, output_dir) or n_actions
        elif model_name == "ppo":
            export_ppo(args.platform, n_actions, output_dir)
        elif model_name == "xgboost":
            export_xgboost(args.platform, n_actions, output_dir)
        print()

    # Write config header with the final n_actions value
    write_config_h(actual_n_actions, args.platform, output_dir)
    print(f"Config: AI_SCHED_N_ACTIONS={actual_n_actions}")
    print("Done.")
